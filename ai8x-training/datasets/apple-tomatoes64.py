"""
Apple-Tomatoes Dataset for 64x64 images
"""
import os
import sys
...
class AppleTomatoes64Dataset(Dataset):
    """
    Apple vs Tomatoes classification dataset.
    ...
    def __print_download_manual(self):
        print("******************************************")
        print("Apple-Tomatoes dataset not found! Please make sure your data is in the correct directory structure:")
        print("Make sure that images are in the following directory structure:")
        print("  \'data/apple-tomatoes/train/apple\'")
        print("  \'data/apple-tomatoes/train/tomatoes\'")
        print("  \'data/apple-tomatoes/test/apple\'")
        print("  \'data/apple-tomatoes/test/tomatoes\'")
        print("******************************************")
...
def get_apple_tomatoes64_dataset(data, load_train, load_test):
    """
    Load the Apple-Tomatoes dataset.
    ...
    if load_train:
        try:
            train_dataset = AppleTomatoes64Dataset(root_dir=data_dir, d_type='train',
    ...
    if load_test:
        try:
            test_dataset = AppleTomatoes64Dataset(root_dir=data_dir, d_type='test', transform=transform)
...
datasets = [
    {
        'name': 'apple-tomatoes64',
        'input': (3, 64, 64),
        'output': ('apple', 'tomatoes'),
        'loader': get_apple_tomatoes64_dataset,
    },
]