"""
Pizza-Not-Pizza Dataset for 64x64 images
"""
import os
import sys
...
class PizzaNotPizza64Dataset(Dataset):
    """
    Pizza vs Not-Pizza classification dataset.
    Optimized for 64x64 input images.

    Args:
    root_dir (string): Root directory of dataset where images are located.
    ...
    def __print_download_manual(self):
        print("******************************************")
        print("Pizza-Not-Pizza dataset not found! Please make sure your data is in the correct directory structure:")
        print("Make sure that images are in the following directory structure:")
        print("  \'data/pizza-not-pizza/train/pizza\'")
        print("  \'data/pizza-not-pizza/train/not_pizza\'")
        print("  \'data/pizza-not-pizza/test/pizza\'")
        print("  \'data/pizza-not-pizza/test/not_pizza\'")
        print("******************************************")
...
def get_pizza_not_pizza64_dataset(data, load_train, load_test):
    """
    Load the Pizza-Not-Pizza dataset.
    ...
    if load_train:
        try:
            train_dataset = PizzaNotPizza64Dataset(root_dir=data_dir, d_type='train',
    ...
    if load_test:
        try:
            test_dataset = PizzaNotPizza64Dataset(root_dir=data_dir, d_type='test', transform=transform)
...
datasets = [
    {
        'name': 'pizza-not-pizza64',
        'input': (3, 64, 64),
        'output': ('pizza', 'not_pizza'),
        'loader': get_pizza_not_pizza64_dataset,
    },
]