"""
Cat-Rabbit Dataset for 64x64 images
"""
import os
import sys
...
class CatRabbit64Dataset(Dataset):
    """
    Cat vs Rabbit classification dataset.
    ...
    def __print_download_manual(self):
        print("******************************************")
        print("Cat-Rabbit dataset not found! Please make sure your data is in the correct directory structure:")
        print("Make sure that images are in the following directory structure:")
        print("  \'data/cat-rabbit/train/cat\'")
        print("  \'data/cat-rabbit/train/rabbit\'")
        print("  \'data/cat-rabbit/test/cat\'")
        print("  \'data/cat-rabbit/test/rabbit\'")
        print("******************************************")
...
def get_cat_rabbit64_dataset(data, load_train, load_test):
    """
    Load the Cat-Rabbit dataset.
    ...
    if load_train:
        try:
            train_dataset = CatRabbit64Dataset(root_dir=data_dir, d_type='train',
    ...
    if load_test:
        try:
            test_dataset = CatRabbit64Dataset(root_dir=data_dir, d_type='test', transform=transform)
...
datasets = [
    {
        ...
        'input': (3, 64, 64),
        'output': ('cat', 'rabbit'),
        'loader': get_cat_rabbit64_dataset,
    },
]