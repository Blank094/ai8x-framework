...
torch._dynamo.config.suppress_errors = True  # Suppress errors and continue in eager mode
...
def main():
    ...
    # CHANGES
    if args.compiler_mode.lower() != 'none':
        msglogger.info('Forcing torch.compile() to use "eager" backend on Windows')
        model = torch.compile(model, backend="eager")  # Force eager mode
    ...
    for epoch in range(start_epoch, ending_epoch):
        ...
        if qat_policy is not None and epoch > 0 and epoch == qat_policy['start_epoch']:
            ...
            if dynamo:
                ...
                # FIX: Force eager mode, just like at line 722,
                # to avoid Windows 'unicodeescape' bug
                model = torch.compile(model, backend="eager")

                # model = torch.compile(model, mode="default", dynamic=False)  # If set to True, try False
        ...
        if run_validation or run_nas_validation:
            ...
            if not args.dr:
                top1, top5, vloss, mAP, precision, recall, f1 = validate(val_loader, model, criterion, [pylogger], args, epoch, tflogger)
            else:
            ...
            update_training_scores_history(perf_scores_history, model, top1, top5, mAP, vloss,
                                           epoch, args, precision, recall, f1)
            ...
            if run_validation:
                ...
                checkpoint_extras = {
                    ...
                    'current_f1': f1,
                    'best_precision': perf_scores_history[0].precision,
                    'best_recall': perf_scores_history[0].recall,
                    'best_f1': perf_scores_history[0].f1}
            else:
                is_best = False
...
def test(test_loader, model, criterion, loggers, args, mode='ckpt', ckpt_name=None, local_rank=0):
    ...
    if mode == 'ckpt':
        ...
        top1, top5, vloss, mAP, precision, recall, f1 = _validate(test_loader, model, criterion, loggers, args)
    else:
    ...
    return top1, top5, vloss, mAP, precision, recall, f1
...
def _validate(data_loader, model, criterion, loggers, args, epoch=-1, tflogger=None):
    ...
    if not args.regression and len(all_targets) > 0:
        precision = precision_score(all_targets, all_preds, average='macro', zero_division=0)
        recall = recall_score(all_targets, all_preds, average='macro', zero_division=0)
        f1 = f1_score(all_targets, all_preds, average='macro', zero_division=0)
        msglogger.info('==> Precision: %.3f    Recall: %.3f    F1: %.3f\n', precision, recall, f1)
...
def update_training_scores_history(perf_scores_history, model, top1, top5, mAP, vloss, epoch,
    ...
    perf_scores_history.append(
        distiller.MutableNamedTuple({'params_nnz_cnt': -params_nnz_cnt,
                                     ...
                                     'epoch': epoch, 'precision': precision, 'recall': recall, 'f1': f1}))
        ...
        for score in perf_scores_history[:args.num_best_scores]:
            if args.num_classes > 5:
                msglogger.info('==> Best [Top1: %.3f   Top5: %.3f   '
                            ...
                            'Params: %d on epoch: %d]',
                            score.top1, score.top5, score.precision, score.recall, score.f1,
                            -score.params_nnz_cnt, score.epoch)
            else:
                msglogger.info('==> Best [Top1: %.3f   '
                            'Precision: %.3f   Recall: %.3f   F1: %.3f   '
                            'Params: %d on epoch: %d]',
                            score.top1, score.precision, score.recall, score.f1,
                            -score.params_nnz_cnt, score.epoch)
